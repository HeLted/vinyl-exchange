namespace VinylExchange.Data.Seeding
{
    ...
    public class RoleSeeder
    {
        ...
        public async Task SeedRoles()
        {
            ...
            if (!adminRoleExists) await this.roleManager.CreateAsync(new VinylExchangeRole(Roles.Admin));

            var userRoleExists = await this.roleManager.RoleExistsAsync(Roles.User);

            if (!userRoleExists) await this.roleManager.CreateAsync(new VinylExchangeRole(Roles.User));
